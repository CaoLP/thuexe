...
class ImageHelper extends AppHelper {
	 ...
	 */
	public function resizedUrl ($file, $width, $height, $quality = 100, $source_dir = null) {
		...
		if ($source_dir != null)
			$file = $source_dir . trim ($file, '/');
		else
			$file = $imageDir . trim ($file, '/');
		$output = 'resized' . DS . $pathinfo['filename'] . '_' . $width . 'x' . $height . '.' . $pathinfo['extension'];
		//		return  $file; die;
